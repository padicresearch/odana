...
pub enum LocalMessage {
    MindedBlock(Block),
    BroadcastTx(Tx),
    StateChanged{ current_head : BlockHeader, mempool : MempoolSnapsot}
}

impl BlockChain {
    pub fn new(storage : Arc<PersistentStorage>, local_mpsc_sender : UnboundedSender<LocalMessage>) -> Result<Self> {
        ...
        let miner = Arc::new(Miner::new(mempool.clone(),utxo.clone(),local_mpsc_sender.clone()));
    ...
    pub fn dispatch(&self, action : StateAction) -> Result<()> {
        match self.state.dispatch(action)? {
            StateEffect::CurrentHeadChanged => {
                if let (Ok(Some(current_head)),Ok(mempool))  = (self.state.get_current_head(),self.state.get_mempool()) {
                    self.local_mpsc_sender.send(LocalMessage::StateChanged{current_head, mempool})?;
                }
...
pub fn start_mining(miner: Arc<Miner>, state: Arc<BlockChainState>, sender: UnboundedSender<LocalMessage>)  {
