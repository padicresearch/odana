        ...
        for res in self.history_log.iter_operations()? {
            ...
            if account == account_id {
                ...
                if let Some(ProofElement::History { .. }) = proof.last() {
                    let d = proof.len() - 1;
                    std::mem::swap(proof.get_mut(d).ok_or(anyhow::anyhow!("poof index not found"))?, &mut ProofElement::History { index: idx as u64 })
                    //proof.push();
                } else {
                    proof.push(ProofElement::History { index: idx as u64 });
                }
...
pub fn validate_account_state(proof: &Vec<ProofElement>, morph: &Morph) -> Result<(AccountState, u64), Error>{
